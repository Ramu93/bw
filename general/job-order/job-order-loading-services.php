<?php
	...
	function createJobOrder(){
		...
		$query = "INSERT INTO general_joborder_loading(pdr_id, space_occupied_after, supervisor_name, loading_type, equipment_ref_number, no_of_labors, loading_time, created_date) VALUES ('$pdrId', '$spaceOccupiedAfter', '$supervisorName', '$loadingType', '$equipmentRefNumber', '$noOfLabors', '$loadingTime', '".date("Y-m-d")."')";
	...
	function addOGPEntry($jlId){
		global $dbc;
		$query = "INSERT INTO general_ogp_loading (jl_id, created_date) VALUES ('$jlId', '".date("Y-m-d")."')";
		mysqli_query($dbc, $query);
	}

	function addGDNEntry($pdrId){
		global $dbc;
		$query = "INSERT INTO general_good_delivery_note (pdr_id, created_date) VALUES ('$pdrId', '".date("Y-m-d")."')";
		mysqli_query($dbc, $query);
	}
