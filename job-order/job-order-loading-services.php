<?php
	...
	function getDataList(){
		...
		switch ($dataType) {
			case 'pdr_id':
				$query = "SELECT pdr_id as 'data_item' FROM despatch_request WHERE status='igp_created' AND document_verified='yes'";
			break;
			case 'boe_number':
				$query = "SELECT boe_number as 'data_item' FROM despatch_request WHERE status='igp_created' AND document_verified='yes'";
			break;
			case 'bond_number':
				$query = "SELECT bond_number as 'data_item' FROM despatch_request WHERE status='igp_created' AND document_verified='yes'";
			break;
			default:
			break;
		}
	...
	function getSelectedDataDetails(){
		...
		if(mysqli_num_rows($result) > 0) {
			...
			$getSpaceQuery = 'SELECT space_occupied FROM good_receipt_note WHERE sac_par_table=\'' .$out['sac_par_table']. '\' AND sac_par_id=\'' .$out['sac_par_id']. '\'';
