<?php
	...
	function getDataList(){
		...
		switch ($dataType) {
			case 'pdr_id':
				$query = "SELECT pdr_id as 'data_item' FROM bonded_despatch_request WHERE status='approved'";
			break;
			case 'boe_number':
	...
	function generateIGP(){
		...
		$query = "INSERT INTO bonded_igp_loading (pdr_id, data_type, data_value, vehicle_number, driver_name, driving_license,time_in, entry_date, created_date) VALUES ('$pdrId', '$dataType', '$dataValue', '$vehicleNumber', '$driverName', '$drivingLicense', '$timeIn', '".date("Y-m-d")."', '".date("Y-m-d")."')";
	...
	function getPDRItemsCount(){
		...
		$query = "SELECT sum(despatch_qty) as 'qty' FROM bonded_pdr_items WHERE pdr_id='$pdrId'";
		$result = mysqli_query($dbc, $query);
		$qty = 0;
		if(mysqli_num_rows($result) > 0){
	...
	function setVehicleLeftTimeStamp(){
		...
		$query = "UPDATE bonded_ogp_loading SET exit_time=now(), status='".OGP_COMPLETE_STATUS."' WHERE ogp_lo_id='$ogpId'";
		if(mysqli_query($dbc, $query)){
