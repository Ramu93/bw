<?php
	...
	function getUnits(){
		global $dbc;
		$query = "SELECT * FROM bonded_unit_master";
		$result = mysqli_query($dbc, $query);
		//file_put_contents("querylog.log", print_r( $row, true ));
		$out = array();
		if(mysqli_num_rows($result) > 0) {
	...
	function addUnit(){
		...
		$query = "INSERT INTO bonded_unit_master (unit_name) VALUES ('$unitName')";
		if(mysqli_query($dbc, $query)){
			return array('infocode' => 'success');
		} else {
			return array('infocode' => 'failure');
	...
	function editUnit(){
		...
		$query = "UPDATE bonded_unit_master SET unit_name='$unitName' WHERE unit_id='$unitId'";
		if(mysqli_query($dbc, $query)){
			return array('infocode' => 'success');
		} else {
			return array('infocode' => 'failure');
	...
	function  deleteUnit(){
		global $dbc;
		$unitId = mysqli_real_escape_string($dbc, trim($_POST['unit_id']));
		$query = "DELETE FROM bonded_unit_master WHERE unit_id='$unitId'";
		if(mysqli_query($dbc, $query)){
			return array('infocode' => 'success');
		} else {
			return array('infocode' => 'failure');
