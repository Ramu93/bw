      ...
      <div class="box-body">
        <form  id="discount_tariff_form" name="discount_tariff_form" method="post" class="validator-form1" action="" onsubmit="return false;">
                    ...
                    if(mysqli_num_rows($result) > 0){
                      ...
                      while($row = mysqli_fetch_assoc($result)){
                        echo '<tr>
                                ...
                                <td><input type="text" name="discount_percentage[]" onkeyup="changeDiscountRate('.$row['tariff_master_id'].')" id="'.$row['tariff_master_id'].'_discount_percentage"></td>
                                <td><input type="text" name="'.$row['tariff_master_id'].'_discount_amount" id="'.$row['tariff_master_id'].'_discount_amount" onkeyup="changeDiscountPercentage('.$row['tariff_master_id'].')" >
                                  <input type="hidden" name="'.$row['tariff_master_id'].'_discount_rate" id="'.$row['tariff_master_id'].'_discount_rate" onkeyup="changeDiscountPercentage('.$row['tariff_master_id'].')" value="'.$row['price_per_unit'].'" >
                                </td>
                              </tr>';
                      }
                    }
          ...
          <div class="row">
            ...
            <div class="col-md-4 col-sm-4">
              <input type="button" id="add_discount_tariff_btn" class="btn btn-primary btn-block" onclick="addDiscountTariff()" value="Add">
            </div>
            <div class="col-md-4 col-sm-4">
...
$('#add_discount_tariff_btn').prop('disabled', 'disabled');
